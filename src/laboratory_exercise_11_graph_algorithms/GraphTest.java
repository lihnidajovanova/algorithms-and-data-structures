    ...
    public List<Edge> prim(int startVertexIndex) {
        ...
        for (int i = 0; i < numVertices; i++) {
            if (isEdge(startVertexIndex, i)) {
                q.add(new Edge(startVertexIndex, i, adjacencyMatrix[startVertexIndex][i]));
            }
        }
            ...
            if (!included[e.getTo()]) {
                ...
                for (int i = 0; i < numVertices; i++) {
                    if (!included[i] && isEdge(e.getTo(), i)) {
                        q.add(new Edge(e.getTo(), i, adjacencyMatrix[e.getTo()][i]));
                    }
                }
            }
        }
...
public class GraphTest {
    public static void main(String[] args) {
        ...
        Graph<String> graph = new Graph<>(numVertices);
        ...
        String lastSubjectPassed = scanner.nextLine();
        System.out.println(graph.prim(map.get(lastSubjectPassed)).stream().mapToInt(Edge::getWeight).sum());
    }
}
